...
func (grp *genericRPMWrappedJavaArchiveParser) parseRPMJavaArchive(
	ctx context.Context, resolver file.Resolver, _ *generic.Environment, reader file.LocationReadCloser,
	...
	rpmPackages, _, err = _cataloger.Catalog(ctx, resolver)
	if err != nil {
	...
	for _, rpmPackage := range rpmPackages {
		_packages, _relationships, err := discoverPkgsFromRPM(ctx, reader.Location, archivePath, contentPath, rpmPackage, grp.cfg)
		if err == nil {
...
func discoverPkgsFromRPM(
	ctx context.Context, location file.Location, archivePath, contentPath string, parentPkg pkg.Package,
	cfg ArchiveCatalogerConfig,
) ([]pkg.Package, []artifact.Relationship, error) {
	...
	var relationships []artifact.Relationship
	_packages, _, err := discoverPkgsFromOpeners(ctx, location, openers, &parentPkg, cfg)
	if err == nil {
